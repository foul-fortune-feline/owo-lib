...
public class TextOps {
    ...
    public static MutableText concat(Text prefix, Text text) {
        return MutableText.of(new LiteralTextContent("")).append(prefix).append(text);
    }
     ...
     * @return The colored text, specifically a {@link MutableText}
     */
    public static MutableText withColor(String text, int color) {
        return MutableText.of(new LiteralTextContent(text)).setStyle(Style.EMPTY.withColor(color));
    }
    ...
    public static MutableText translateWithColor(String text, int color) {
        return MutableText.of(new TranslatableTextContent(text)).setStyle(Style.EMPTY.withColor(color));
    }
    ...
    public static MutableText withFormatting(String text, Formatting... formatting) {
        ...
        var textBase = MutableText.of(new LiteralTextContent(textPieces[0])).formatted(formatting[0]);

        for (int i = 1; i < textPieces.length; i++) {
            textBase.append(MutableText.of(new LiteralTextContent(textPieces[i])).formatted(formatting[i]));
        }
